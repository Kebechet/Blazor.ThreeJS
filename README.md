...
**32** of its constant groups, carrying 1686 public members and 171 enum members. Property names,
...
Two of the wire format's op kinds answer with a value: **read** invokes a three.js method, and **get**
reads a property. Both travel inside the batch they were recorded in, so either always observes the
writes made before it. They carry **values** - numbers, booleans, strings, and the 19 hand-written math
types, tagged exactly as they are sent in the other direction.

On the generated classes that reaches **427 members**, each emitted as `…Async` returning a `Task<T>`:

- **264 methods** - focal length and effective field of view, elapsed time, curve lengths, instance
  matrices and colours, vertex positions, layer tests.
- **163 read-only properties** - `uuid`, `instanceCount`, `unusedVertexCount`, and three.js's own
  `isMesh`-style type tags. Read on demand rather than mirrored, because three.js is the only side that
  ever assigns them: a C# property would imply the mirror knew the value without asking.

It does **not** carry handles, and that is what still puts members out of reach:

- **1 methods returning a three.js object** - `clone`, `getObjectByName`, `getRenderTarget`,
...
- **54 of the 194 generated classes have a narrower constructor.** 96 trailing optional parameters
...
- **`T | T[]` maps to `T`** in 12 declared types, so a mesh with several materials is not expressible.
