...
/// <summary>
/// A material for non-shiny surfaces, without specular highlights. The material uses a
/// non-physically based [Lambertian](https://en.wikipedia.org/wiki/Lambertian_reflectance) model
/// for calculating reflectance. This can simulate some surfaces (such as untreated wood or stone)
/// well, but cannot simulate shiny surfaces with specular highlights (such as varnished wood).
/// <c>MeshLambertMaterial</c> uses per-fragment shading. Due to the simplicity of the reflectance
/// and illumination models, performance will be greater when using this material over the
/// <see cref="MeshPhongMaterial"/>, <see cref="MeshStandardMaterial"/> or
/// <see cref="MeshPhysicalMaterial"/>, at the cost of some graphical accuracy. The JavaScript-side
/// <c>THREE.MeshLambertMaterial</c>.
/// </summary>
public sealed class MeshLambertMaterial : Material
{
	...
	/// <summary>Constructs a new mesh lambert material.</summary>
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>MeshLambertMaterial</c> under the handle the browser
	/// minted for it. No create op is emitted: the object already exists, and this mirror's job is to
	/// name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal MeshLambertMaterial(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Color = new Color(1f, 1f, 1f);
		Color.OnChange = () =>
	...
	/// <summary>Name of the corresponding three.js constructor, <c>THREE.MeshLambertMaterial</c>.</summary>
	protected override string ThreeTypeName
	{
		get { return "MeshLambertMaterial"; }
	}
	...
	/// the value <c>isMeshLambertMaterial</c> held.
	/// </summary>
	/// <returns>The value <c>isMeshLambertMaterial</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsMeshLambertMaterialAsync()
	{
		return GetAsync<bool>("isMeshLambertMaterial");
	}

	/// <summary>
	/// Emits the create op for <c>THREE.MeshLambertMaterial</c>, then replays every property written
