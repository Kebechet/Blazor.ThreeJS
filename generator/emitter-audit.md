...
| **defaulted to `float`** | 136 | 0 |

### Inferred — review these (143)
...
| `CubicInterpolant` | `property DefaultSettings_` | `CubicInterpolantSettings` is an interface, and the mirror has no representation for a structural type — only for classes it constructs by handle |
| `CubicInterpolant` | `property settings` | `CubicInterpolantSettings` is an interface, and the mirror has no representation for a structural type — only for classes it constructs by handle |
...
| `Renderer` | `method renderObject` | parameter 'group': `GeometryGroup` is an interface, and the mirror has no representation for a structural type — only for classes it constructs by handle |
