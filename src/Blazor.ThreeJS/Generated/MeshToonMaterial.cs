...
public sealed class MeshToonMaterial : Material
{
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>MeshToonMaterial</c> under the handle the browser minted
	...
	internal MeshToonMaterial(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Color = new Color(1f, 1f, 1f);
		Color.OnChange = () =>
	...
	/// the value <c>isMeshToonMaterial</c> held.
	/// </summary>
	/// <returns>The value <c>isMeshToonMaterial</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsMeshToonMaterialAsync()
	{
		return GetAsync<bool>("isMeshToonMaterial");
	}
