...
/// <summary>
/// A material for shiny surfaces with specular highlights. The material uses a non-physically based
/// [Blinn-Phong](https://en.wikipedia.org/wiki/Blinn-Phong_shading_model) model for calculating
/// reflectance. Unlike the Lambertian model used in the <see cref="MeshLambertMaterial"/> this can
/// simulate shiny surfaces with specular highlights (such as varnished wood).
/// <c>MeshPhongMaterial</c> uses per-fragment shading. Performance will generally be greater when
/// using this material over the <see cref="MeshStandardMaterial"/> or
/// <see cref="MeshPhysicalMaterial"/>, at the cost of some graphical accuracy. The JavaScript-side
/// <c>THREE.MeshPhongMaterial</c>.
/// </summary>
public sealed class MeshPhongMaterial : Material
{
	...
	/// <summary>Constructs a new mesh phong material.</summary>
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>MeshPhongMaterial</c> under the handle the browser minted
	...
	internal MeshPhongMaterial(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Color = new Color(1f, 1f, 1f);
		Color.OnChange = () =>
	...
	/// <summary>Name of the corresponding three.js constructor, <c>THREE.MeshPhongMaterial</c>.</summary>
	protected override string ThreeTypeName
	{
		get { return "MeshPhongMaterial"; }
	}
	...
	/// the value <c>isMeshPhongMaterial</c> held.
	/// </summary>
	/// <returns>The value <c>isMeshPhongMaterial</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsMeshPhongMaterialAsync()
	{
		return GetAsync<bool>("isMeshPhongMaterial");
	}

	/// <summary>
	/// Emits the create op for <c>THREE.MeshPhongMaterial</c>, then replays every property written
