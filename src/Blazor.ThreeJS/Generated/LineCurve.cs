...
public sealed class LineCurve : ThreeObject
{
	...
	/// <param name="v1">The start point. Default is <c>new THREE.Vector2()</c>.</param>
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>LineCurve</c> under the handle the browser minted for it.
	/// No create op is emitted: the object already exists, and this mirror's job is to name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal LineCurve(ThreeBatch batch, int handle)
		: base(handle)
	{
		Batch = batch;
	}
	...
	/// <summary>
	/// Constructor arguments forwarded to <c>THREE.LineCurve</c>: v1, v2. An argument the caller left
	...
	/// <summary>
	/// Read-only flag to check if a given object is of type <see cref="LineCurve"/>. Read-only in
	/// three.js, so it is read on demand rather than mirrored: records a get op, sends it behind every
	/// write already pending, and completes with the value <c>isLineCurve</c> held.
	/// </summary>
	/// <returns>The value <c>isLineCurve</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsLineCurveAsync()
	{
		return GetAsync<bool>("isLineCurve");
	}
	...
	public Task<LineCurve?> CloneAsync()
	{
		return RecordReadObject<LineCurve>("clone", (adoptedBatch, adoptedHandle) => new LineCurve(adoptedBatch, adoptedHandle));
	}
