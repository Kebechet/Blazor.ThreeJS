...
/// <summary>The JavaScript-side <c>THREE.AudioLoader</c>.</summary>
public sealed class AudioLoader : Loader
{
	private readonly LoadingManager? _manager;

	/// <summary>Initializes a new <see cref="AudioLoader"/>.</summary>
	/// <param name="manager">Value forwarded to the <c>manager</c> constructor argument.</param>
	public AudioLoader(LoadingManager? manager = null)
	{
		_manager = manager;
	}
	...
	/// <summary>
	/// Constructor arguments forwarded to <c>THREE.AudioLoader</c>: manager. An argument the caller
	...
	/// <summary>
	/// Attaches the objects <c>THREE.AudioLoader</c> is constructed from, so their create ops reach the
