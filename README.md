...
**32** of its constant groups, carrying 957 public members and 171 enum members. Property names,
...
value**. On the generated classes that puts 206 members out of reach:

- **66 methods whose result is the point of calling them.** Raycast results, bounding-box and
...
- **99 read-only properties**, which C# could only guess at.
- **41 methods returning a fresh object** (`clone`, `removeFromParent`), which would need a handle for
