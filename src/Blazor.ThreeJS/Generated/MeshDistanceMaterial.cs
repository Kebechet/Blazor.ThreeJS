...
public sealed class MeshDistanceMaterial : Material
{
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>MeshDistanceMaterial</c> under the handle the browser
	/// minted for it. No create op is emitted: the object already exists, and this mirror's job is to
	/// name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal MeshDistanceMaterial(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Batch = batch;
	}
	...
	/// the value <c>isMeshDistanceMaterial</c> held.
	/// </summary>
	/// <returns>The value <c>isMeshDistanceMaterial</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsMeshDistanceMaterialAsync()
	{
		return GetAsync<bool>("isMeshDistanceMaterial");
	}
