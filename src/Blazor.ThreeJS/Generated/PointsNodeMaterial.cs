...
public sealed class PointsNodeMaterial : SpriteNodeMaterial
{
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>PointsNodeMaterial</c> under the handle the browser minted
	...
	internal PointsNodeMaterial(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Batch = batch;
	}
	...
	/// the value <c>isPointsNodeMaterial</c> held.
	/// </summary>
	/// <returns>The value <c>isPointsNodeMaterial</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsPointsNodeMaterialAsync()
	{
		return GetAsync<bool>("isPointsNodeMaterial");
	}
