...
public sealed class CatmullRomCurve3 : ThreeObject
{
	...
	/// <summary>
	/// Read-only flag to check if a given object is of type <see cref="CatmullRomCurve3"/>. Read-only
	/// in three.js, so it is read on demand rather than mirrored: records a get op, sends it behind
	/// every write already pending, and completes with the value <c>isCatmullRomCurve3</c> held.
	/// </summary>
	/// <returns>The value <c>isCatmullRomCurve3</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsCatmullRomCurve3Async()
	{
		return GetAsync<bool>("isCatmullRomCurve3");
	}
