...
Generated from `@types/three@0.185.3`: **216 of three.js's 309 core classes** and
**41** of its constant groups, carrying 1887 public members and 252 enum members. Property names,
...
A further 34 public members are generated onto `Object3D`, which is hand-written and so is **not**
one of those 216 classes - its command and query surface is emitted as the other half of a
`partial class`. So `Generated/**` carries 1921 public members in all, and this headline claims
only the ones that sit on a class the generator made.

A further 48 classes have no generated type and are still **reachable** untyped, by name -
...
|---|---|
| **generated** | **216** |
| blocked on something the mirror cannot express yet | 40 |
...
| the class is abstract, so it has no constructor to mirror | 11 |
...
| `src/math/**` value types | 20 | 19 of them ship, hand-ported (`Box2`, `Box3`, `Color`, `Cylindrical`, `Euler`, `Frustum`, `Line3`, `Matrix3`, `Matrix4`, `Plane`, `Quaternion`, `Ray`, `Sphere`, `Spherical`, `SphericalHarmonics3`, `Triangle`, `Vector2`, `Vector3`, `Vector4`); the other 1 do not: `Matrix2`. A math value is arithmetic rather than a signature: the generator has their members but not their behaviour, so each one waits on a hand port |
...
On the generated classes that reaches **480 members**:

- **378 answer with a value** - 203 methods (focal length and effective field of view, elapsed time,
  curve lengths, instance matrices and colours, vertex positions, layer tests) and 175 read-only
...
- **92 answer with a mirrored object** - `Task<T?>` over the generated type, adopted under the handle the
...
- **2 answer with an object no generated class mirrors** - `Task<Primitive?>`, the same untyped wrapper
...
- **8 answer nothing at all** - a bare `Task`, awaited for *when* rather than for what.
...
- **1 member whose result is neither** - an array of objects, say, which would need a handle minted per
  element rather than one for the result.
...
The 40 blocked classes account for themselves as follows:
...
- **15 are absent from the shipped three.js bundle.** Not a mapping decision: `THREE[name]` is
  ...
  escape hatch either.
- **14 lose no capability**, listed below. They are abstract bases whose concrete subclasses all
...
- **11 are neither**, and are a genuine gap nobody has written a route to yet.
...
⚠️ 1 entries above describe classes that are no longer blocked and should be removed from
`EmitterConfig.BlockedClassWorkarounds`: `InstancedInterleavedBuffer`.

### Where a generated type is narrower than three.js

- **53 of the 216 generated classes have a narrower constructor.** 94 trailing optional parameters
...
- **A union in a required parameter becomes one overload per arm** - 5 members carry
  10 signatures between them, so `BufferGeometry.SetIndex` takes either a `BufferAttribute` or an
...
| | classes |
|---|---|
| **generated, and typed** | **216** |
| reachable by name, untyped | 48 |
