...
Generated from `@types/three@0.185.3`: **198 of three.js's 309 core classes** and
**41** of its constant groups, carrying 1773 public members and 252 enum members. Property names,
...
one of those 198 classes - its command and query surface is emitted as the other half of a
`partial class`. So `Generated/**` carries 1807 public members in all, and this headline claims
only the ones that sit on a class the generator made.

A further 66 classes have no generated type and are still **reachable** untyped, by name -
...
|---|---|
| **generated** | **198** |
| blocked on something the mirror cannot express yet | 51 |
...
On the generated classes that reaches **413 members**:

- **345 answer with a value** - 181 methods (focal length and effective field of view, elapsed time,
  curve lengths, instance matrices and colours, vertex positions, layer tests) and 164 read-only
...
- **49 members typed as a DOM or TypeScript lib type** - C# holds no `HTMLCanvasElement` to hand over,
...
The 51 blocked classes account for themselves as follows:
...
- **15 are absent from the shipped three.js bundle.** Not a mapping decision: `THREE[name]` is
  ...
  escape hatch either.
- **24 lose no capability**, listed below. They are abstract bases whose concrete subclasses all
...
- **12 are neither**, and are a genuine gap nobody has written a route to yet.
...
- **53 of the 198 generated classes have a narrower constructor.** 94 trailing optional parameters
...
| | classes |
|---|---|
| **generated, and typed** | **198** |
| reachable by name, untyped | 66 |
