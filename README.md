...
Generated from `@types/three@0.185.3`: **189 of three.js's 309 core classes** and
**32** of its constant groups, carrying 1315 public members and 171 enum members. Property names,
...
|---|---|
| **generated** | **189** |
| blocked on something the mirror cannot express yet | 60 |
...
| obstacle | classes |
|---|---|
| a union of several real alternatives, which one C# parameter cannot express | 17 |
| the class is abstract, so it has no constructor to mirror | 12 |
| a TypeScript lib or DOM type; C# holds no browser object and the wire has no encoding for one | 9 |
| a type alias that is neither a constant group nor a rename of a mapped type | 6 |
...
| the class is never exported, so the applier cannot reach it on `THREE` | 2 |
| its C# base requires constructor arguments the generated class has nothing to supply | 2 |
...
### ⚠️ Nothing reads back

The wire format has six op kinds - create, set, call, add, remove, dispose - and **none of them returns a
value**. On the generated classes that puts 329 members out of reach:

- **109 methods whose result is the point of calling them.** Raycast results, bounding-box and
  bounding-sphere computation, world position / rotation / scale, `getObjectByName` - anything that hands
  data back from JavaScript cannot be called at all.
- **172 read-only properties**, which C# could only guess at.
- **48 methods returning a fresh object** (`clone`, `removeFromParent`), which would need a handle for
  something JavaScript created.

This is a property of the wire format rather than of the generator: it is the same limitation whether a
class is generated or hand-written, and closing it needs a read op, not more coverage.

### Where a generated type is narrower than three.js

- **67 of the 189 generated classes have a narrower constructor.** 133 trailing optional parameters
