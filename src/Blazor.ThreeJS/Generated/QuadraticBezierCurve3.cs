...
/// JavaScript-side <c>THREE.QuadraticBezierCurve3</c>.
/// </summary>
/// <seealso href="https://threejs.org/docs/index.html#api/en/extras/curves/QuadraticBezierCurve3">Official Documentation</seealso>
/// <seealso href="https://github.com/mrdoob/three.js/blob/master/src/extras/curves/QuadraticBezierCurve3.js">Source</seealso>
public sealed class QuadraticBezierCurve3 : ThreeObject
{
	...
	/// <param name="v0">The start point. Default is <c>new THREE.Vector3()</c>.</param>
	/// <param name="v1">The control point. Default is <c>new THREE.Vector3()</c>.</param>
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>QuadraticBezierCurve3</c> under the handle the browser
	/// minted for it. No create op is emitted: the object already exists, and this mirror's job is to
	/// name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal QuadraticBezierCurve3(ThreeBatch batch, int handle)
		: base(handle)
	{
		Batch = batch;
	}

	/// <summary>Name of the corresponding three.js constructor, <c>THREE.QuadraticBezierCurve3</c>.</summary>
	protected override string ThreeTypeName
	{
		get { return "QuadraticBezierCurve3"; }
	}

	/// <summary>
	/// Constructor arguments forwarded to <c>THREE.QuadraticBezierCurve3</c>: v0, v1, v2. An argument
	...
	/// <summary>
	/// Read-only flag to check if a given object is of type <see cref="QuadraticBezierCurve3"/>.
	/// Read-only in three.js, so it is read on demand rather than mirrored: records a get op, sends it
	/// behind every write already pending, and completes with the value <c>isQuadraticBezierCurve3</c>
	/// held.
	/// </summary>
	/// <returns>The value <c>isQuadraticBezierCurve3</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsQuadraticBezierCurve3Async()
	{
		return GetAsync<bool>("isQuadraticBezierCurve3");
	}
	...
	public Task<QuadraticBezierCurve3?> CloneAsync()
	{
		return RecordReadObject<QuadraticBezierCurve3>("clone", (adoptedBatch, adoptedHandle) => new QuadraticBezierCurve3(adoptedBatch, adoptedHandle));
	}
	...
	/// <summary>
	/// Emits the create op for <c>THREE.QuadraticBezierCurve3</c>, then replays every property written
