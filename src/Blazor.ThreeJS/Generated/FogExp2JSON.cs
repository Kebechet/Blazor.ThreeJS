...
/// <summary>
/// The shape three.js calls <c>FogExp2JSON</c>. A plain value rather than a handle-backed object:
...
/// </summary>
public sealed record FogExp2JSON : IThreeStructure
{
	...
	/// <summary>three.js's <c>density</c>.</summary>
	...
	IReadOnlyDictionary<string, object?> IThreeStructure.ToWireMembers()
	{
		...
		members["name"] = Name;
		members["color"] = Color;
		members["density"] = Density;

		return members;
	}

	/// <summary>
	/// Builds a <c>FogExp2JSON</c> from the members the applier sent back. A member three.js did not
	...
	IThreeStructure IThreeStructure.FromWireMembers(IReadOnlyDictionary<string, JsonElement> members)
	{
		return new FogExp2JSON
		{
			...
			Density = members.TryGetValue("density", out var densityElement) ? ThreeValue.Decode<float>(densityElement) : Density
		};
	}
}
