...
public sealed class PointsMaterial : Material
{
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>PointsMaterial</c> under the handle the browser minted for
	/// it. No create op is emitted: the object already exists, and this mirror's job is to name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal PointsMaterial(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Color = new Color(1f, 1f, 1f);
		Color.OnChange = () =>
	...
	/// the value <c>isPointsMaterial</c> held.
	/// </summary>
	/// <returns>The value <c>isPointsMaterial</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsPointsMaterialAsync()
	{
		return GetAsync<bool>("isPointsMaterial");
	}
