...
/// <summary>
/// The shape three.js calls <c>XRHandInputState</c>. A plain value rather than a handle-backed
...
/// </summary>
public sealed record XRHandInputState : IThreeStructure
{
	/// <summary>three.js's <c>pinching</c>.</summary>
	public bool Pinching { get; init; }
	...
	IReadOnlyDictionary<string, object?> IThreeStructure.ToWireMembers()
	{
		...
		members["pinching"] = Pinching;

		return members;
	}

	/// <summary>
	/// Builds a <c>XRHandInputState</c> from the members the applier sent back. A member three.js did
	...
	{
		return new XRHandInputState
		{
			Pinching = members.TryGetValue("pinching", out var pinchingElement) ? ThreeValue.Decode<bool>(pinchingElement, context) : Pinching
		};
	}
}
