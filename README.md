...
Generated from `@types/three@0.185.3`: **226 of three.js's 309 core classes** and
**44** of its constant groups, carrying 2223 public members and 263 enum members. Property names,
...
A further 36 public members are generated onto `Object3D`, which is hand-written and so is **not**
one of those 226 classes - its command and query surface is emitted as the other half of a
`partial class`. So `Generated/**` carries 2259 public members in all, and this headline claims
only the ones that sit on a class the generator made.

A further 38 classes have no generated type and are still **reachable** untyped, by name -
...
| blocked on something the mirror cannot express yet | 30 |
...
On the generated classes that reaches **539 members**:

- **426 answer with a value** - 241 methods (focal length and effective field of view, elapsed time,
  curve lengths, instance matrices and colours, vertex positions, layer tests) and 185 read-only
...
- **102 answer with a mirrored object** - `Task<T?>` over the generated type, adopted under the handle the
...
- **44 members typed as a DOM or TypeScript lib type** - C# holds no `HTMLCanvasElement` to hand over,
...
- **5 members whose result is neither** - an array of objects, say, which would need a handle minted per
...
The 30 blocked classes account for themselves as follows:
...
- **15 are absent from the shipped three.js bundle.** Not a mapping decision: `THREE[name]` is
  ...
  escape hatch either.
- **10 lose no capability**, listed below. They are abstract bases whose concrete subclasses all
...
⚠️ 5 entries above describe classes that are no longer blocked and should be removed from
`EmitterConfig.BlockedClassWorkarounds`: `CompressedCubeTexture`, `CompressedTexture`, `DataTextureLoader`, `InstancedInterleavedBuffer`, `Light`.

### Where a generated type is narrower than three.js

- **59 of the 226 generated classes have a narrower constructor.** 109 trailing optional parameters
...
- **`T | T[]` maps to `T`** in 11 declared types, so a mesh with several materials is not expressible.
...
| | classes |
|---|---|
| **generated, and typed** | **226** |
| reachable by name, untyped | 38 |
