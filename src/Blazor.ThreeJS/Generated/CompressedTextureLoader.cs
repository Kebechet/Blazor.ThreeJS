...
/// <summary>The JavaScript-side <c>THREE.CompressedTextureLoader</c>.</summary>
public sealed class CompressedTextureLoader : Loader
{
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>CompressedTextureLoader</c> under the handle the browser
	/// minted for it. No create op is emitted: the object already exists, and this mirror's job is to
	/// name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal CompressedTextureLoader(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Batch = batch;
	}

	/// <summary>Name of the corresponding three.js constructor, <c>THREE.CompressedTextureLoader</c>.</summary>
	protected override string ThreeTypeName
	{
		get { return "CompressedTextureLoader"; }
	}

	/// <summary>
	/// Constructor arguments forwarded to <c>THREE.CompressedTextureLoader</c>: manager. An argument
	...
	/// <summary>
	/// Attaches the objects <c>THREE.CompressedTextureLoader</c> is constructed from, so their create
