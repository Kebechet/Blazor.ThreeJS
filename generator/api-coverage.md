...
| `InstancedMesh` | `property instanceMatrix` | `UnreachableBaseConstructor` | `InstancedBufferAttribute` is not an emitted class: its C# base `BufferAttribute` has a constructor requiring `array`, `itemSize`, and a generated class carries only its own constructor arguments — it has nothing to chain with |
| `InstancedMesh` | `property instanceColor` | `UnreachableBaseConstructor` | `InstancedBufferAttribute` is not an emitted class: its C# base `BufferAttribute` has a constructor requiring `array`, `itemSize`, and a generated class carries only its own constructor arguments — it has nothing to chain with |
...
| `Raycaster` | `method intersectObjects` | `DomOrLibType` | return type: `Array<Intersection<TIntersected>>` is a TypeScript lib type; C# holds no browser object and the wire format has no encoding for one |
...
| `Renderer` | `method getArrayBufferAsync` | `DomOrLibType` | return type: `Promise<ArrayBuffer>` is a TypeScript lib type; C# holds no browser object and the wire format has no encoding for one |
...
| `Renderer` | `method initTextureAsync` | `DomOrLibType` | return type: `Promise<void>` is a TypeScript lib type; C# holds no browser object and the wire format has no encoding for one |
...
| `WebGLLights` | `method setup` | `AbstractClass` | parameter 'lights': `Light[]` is an array whose element type cannot be mapped: `Light` is not an emitted class: the class is abstract, so it has no constructor to mirror |
| `WebGLLights` | `method setupView` | `AbstractClass` | parameter 'lights': `Light[]` is an array whose element type cannot be mapped: `Light` is not an emitted class: the class is abstract, so it has no constructor to mirror |
...
| `WebGLRenderer` | `method readRenderTargetPixelsAsync` | `DomOrLibType` | return type: `Promise<TypedArray>` is a TypeScript lib type; C# holds no browser object and the wire format has no encoding for one |
| `WebGLShadowMap` | `method render` | `AbstractClass` | parameter 'shadowsArray': `Light[]` is an array whose element type cannot be mapped: `Light` is not an emitted class: the class is abstract, so it has no constructor to mirror |
