...
public sealed class WebGLCapabilities : ThreeObject
{
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>WebGLCapabilities</c> under the handle the browser minted
	...
	internal WebGLCapabilities(ThreeBatch batch, int handle)
		: base(handle)
	{
		_backend = default!;
	...
	/// <summary>
	/// Returns the maximum anisotropy texture filtering value. This value depends on the device and is
	/// reported by the <c>EXT_texture_filter_anisotropic</c> WebGL extension. Records a read op, sends
	/// it behind every write already pending, and completes with what <c>getMaxAnisotropy</c> returned.
	/// </summary>
	...
	/// <summary>
	/// Returns the maximum number of bytes available for uniform buffers. Records a read op, sends it
	/// behind every write already pending, and completes with what <c>getUniformBufferLimit</c>
	/// returned.
	/// </summary>
	/// <returns>The value <c>getUniformBufferLimit</c> returned, once the JavaScript side has answered.</returns>
	public Task<float> GetUniformBufferLimitAsync()
	{
		return RecordRead<float>("getUniformBufferLimit");
	}
