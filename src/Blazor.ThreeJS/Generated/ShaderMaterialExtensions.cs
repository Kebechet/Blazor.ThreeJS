...
/// <summary>
/// The shape three.js calls <c>ShaderMaterialExtensions</c>. A plain value rather than a
...
/// </summary>
public sealed record ShaderMaterialExtensions : IThreeStructure
{
	/// <summary>three.js's <c>clipCullDistance</c>.</summary>
	public bool ClipCullDistance { get; init; }

	/// <summary>three.js's <c>multiDraw</c>.</summary>
	public bool MultiDraw { get; init; }
	...
	IReadOnlyDictionary<string, object?> IThreeStructure.ToWireMembers()
	{
		...
		members["clipCullDistance"] = ClipCullDistance;
		members["multiDraw"] = MultiDraw;

		return members;
	}

	/// <summary>
	/// Builds a <c>ShaderMaterialExtensions</c> from the members the applier sent back. A member
	...
	{
		return new ShaderMaterialExtensions
		{
			ClipCullDistance = members.TryGetValue("clipCullDistance", out var clipCullDistanceElement) ? ThreeValue.Decode<bool>(clipCullDistanceElement, context) : ClipCullDistance,
			MultiDraw = members.TryGetValue("multiDraw", out var multiDrawElement) ? ThreeValue.Decode<bool>(multiDrawElement, context) : MultiDraw
		};
	}
}
