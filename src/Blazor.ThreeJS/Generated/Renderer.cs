...
/// <summary>Base class for renderers. The JavaScript-side <c>THREE.Renderer</c>.</summary>
public class Renderer : ThreeObject
{
	...
	/// <summary>Constructs a new renderer.</summary>
	/// <param name="backend">The backend the renderer is targeting (e.g. WebGPU or WebGL 2).</param>
	...
	/// <summary>
	/// Initializes a <c>Renderer</c> whose subclass supplies its constructor arguments on the
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>Renderer</c> under the handle the browser minted for it.
	/// No create op is emitted: the object already exists, and this mirror's job is to name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal Renderer(ThreeBatch batch, int handle)
		: base(handle)
	{
		_backend = default!;
	...
	/// <summary>Name of the corresponding three.js constructor, <c>THREE.Renderer</c>.</summary>
	protected override string ThreeTypeName
	{
		get { return "Renderer"; }
	}

	/// <summary>Constructor arguments forwarded to <c>THREE.Renderer</c>: backend.</summary>
	...
	/// <summary>
	/// A reference to the current backend. Writing it records a <c>backend</c> property write once this
	...
	public Task<Renderer?> InitAsync()
	{
		return RecordReadObject<Renderer>("init", (adoptedBatch, adoptedHandle) => new Renderer(adoptedBatch, adoptedHandle));
	}
	...
	/// <summary>
	/// Attaches the objects <c>THREE.Renderer</c> is constructed from, so their create ops reach the
