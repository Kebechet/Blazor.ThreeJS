...
/// <summary>
/// Create a <see cref="Texture"/> to apply to a surface or as a reflection or refraction map. The
/// JavaScript-side <c>THREE.Texture</c>.
/// </summary>
/// <remarks>
/// After the initial use of a texture, its **dimensions**, <c>format</c>, and <c>type</c> cannot be
/// changed Instead, call <c>.dispose()</c> on the <see cref="Texture"/> and instantiate a new
/// <see cref="Texture"/>.
/// </remarks>
/// <seealso href="https://threejs.org/examples/#webgl_materials_texture_filters">webgl materials texture filters</seealso>
/// <seealso href="https://threejs.org/docs/index.html#api/en/constants/Textures">Texture Constants</seealso>
/// <seealso href="https://threejs.org/docs/index.html#api/en/textures/Texture">Official Documentation</seealso>
...
public class Texture : EventDispatcher
{
	...
	/// **U** and **V**. Mirrored as an instance this object owns: mutating it records a write of
	/// <c>offset</c>.
	/// </summary>
	...
	/// Mirrored as an instance this object owns: mutating it records a write of <c>repeat</c>.
	/// </summary>
	public Vector2 Repeat { get; }

	/// <summary>
	/// The point around which rotation occurs. Mirrored as an instance this object owns: mutating it
	/// records a write of <c>center</c>.
	/// </summary>
	...
	public Texture()
	{
		...
		Offset.OnChange = () =>
		{
			...
			RecordSet("offset", Offset);
		};

		Repeat = new Vector2();
		Repeat.OnChange = () =>
		{
			_isRepeatWritten = true;
			RecordSet("repeat", Repeat);
		};
	...
	/// <summary>
	/// Adopts an existing JavaScript-side <c>Texture</c> under the handle the browser minted for it. No
	/// create op is emitted: the object already exists, and this mirror's job is to name it.
	/// </summary>
	/// <param name="batch">Batch this object's writes record into.</param>
	/// <param name="handle">Negative handle the JavaScript side registered the object under.</param>
	internal Texture(ThreeBatch batch, int handle)
		: base(batch, handle)
	{
		Offset = new Vector2();
		Offset.OnChange = () =>
	...
	/// <summary>Name of the corresponding three.js constructor, <c>THREE.Texture</c>.</summary>
	...
	/// <summary>
	/// This defines how the <see cref="Texture"/> is wrapped *horizontally* and corresponds to **U** in
	...
	/// <summary>
	/// This defines how the <see cref="Texture"/> is wrapped *vertically* and corresponds to **V** in
	...
	/// <summary>
	/// How the <see cref="Texture"/> is sampled when a texel covers more than one pixel. Writing it
	...
	/// <summary>
	/// How the <see cref="Texture"/> is sampled when a texel covers less than one pixel. Writing it
	...
	public void Copy(Texture source)
	{
	...
	/// <summary>
	/// Read-only flag to check if a given object is of type <see cref="Texture"/>. Read-only in
	/// three.js, so it is read on demand rather than mirrored: records a get op, sends it behind every
	/// write already pending, and completes with the value <c>isTexture</c> held.
	/// </summary>
	/// <returns>The value <c>isTexture</c> held, once the JavaScript side has answered.</returns>
	public Task<bool> IsTextureAsync()
	{
		return GetAsync<bool>("isTexture");
	}

	/// <summary>
	/// Unique number for this <see cref="Texture"/> instance. Read-only in three.js, so it is read on
	...
	/// <summary>
	/// The width of the texture in pixels. Read-only in three.js, so it is read on demand rather than
	/// mirrored: records a get op, sends it behind every write already pending, and completes with the
	/// value <c>width</c> held.
	/// </summary>
	/// <returns>The value <c>width</c> held, once the JavaScript side has answered.</returns>
	public Task<float> WidthAsync()
	{
		return GetAsync<float>("width");
	}

	/// <summary>
	/// The height of the texture in pixels. Read-only in three.js, so it is read on demand rather than
	/// mirrored: records a get op, sends it behind every write already pending, and completes with the
	/// value <c>height</c> held.
	/// </summary>
	/// <returns>The value <c>height</c> held, once the JavaScript side has answered.</returns>
	public Task<float> HeightAsync()
	{
		return GetAsync<float>("height");
	}

	/// <summary>
	/// The depth of the texture in pixels. Read-only in three.js, so it is read on demand rather than
	/// mirrored: records a get op, sends it behind every write already pending, and completes with the
	/// value <c>depth</c> held.
	/// </summary>
	/// <returns>The value <c>depth</c> held, once the JavaScript side has answered.</returns>
	public Task<float> DepthAsync()
	{
		return GetAsync<float>("depth");
	}

	/// <summary>
	/// Transform the **UV** based on the value of this texture's <c>.offset</c>, <c>.repeat</c>,
	/// <c>.wrapS</c>, <c>.wrapT</c> and <c>.flipY</c> properties. Records a read op, sends it behind
	/// every write already pending, and completes with what <c>transformUv</c> returned.
	/// </summary>
	/// <param name="uv">Value forwarded to the <c>uv</c> argument.</param>
	/// <returns>The value <c>transformUv</c> returned, once the JavaScript side has answered.</returns>
	public Task<Vector2> TransformUvAsync(Vector2 uv)
	{
		return RecordRead<Vector2>("transformUv", uv);
	}

	/// <summary>
	/// Make copy of the texture. Note this is not a "deep copy", the image is shared. Cloning the
	/// texture automatically marks it for texture upload. Records a read op, sends it behind every
	...
	public Task<Texture?> CloneAsync()
	{
		return RecordReadObject<Texture>("clone", (adoptedBatch, adoptedHandle) => new Texture(adoptedBatch, adoptedHandle));
	}

	/// <summary>
	/// Emits the create op for <c>THREE.Texture</c>, then replays every property written before this
	...
	internal override void EmitCreate(ThreeBatch batch)
	{
		...
		if (_isOffsetWritten)
		{
			batch.Set(Handle, "offset", ThreeValue.Encode(Offset));
		}

		if (_isRepeatWritten)
		{
			batch.Set(Handle, "repeat", ThreeValue.Encode(Repeat));
		}
