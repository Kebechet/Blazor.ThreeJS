...
Generated from `@types/three@0.185.3`: **224 of three.js's 309 core classes** and
...
one of those 224 classes - its command and query surface is emitted as the other half of a
...
A further 40 classes have no generated type and are still **reachable** untyped, by name -
...
| blocked on something the mirror cannot express yet | 32 |
...
On the generated classes that reaches **475 members**:

- **371 answer with a value** - 203 methods (focal length and effective field of view, elapsed time,
  curve lengths, instance matrices and colours, vertex positions, layer tests) and 168 read-only
...
The 32 blocked classes account for themselves as follows:
...
- **15 are absent from the shipped three.js bundle.** Not a mapping decision: `THREE[name]` is
  ...
  escape hatch either.
- **12 lose no capability**, listed below. They are abstract bases whose concrete subclasses all
...
⚠️ 3 entries above describe classes that are no longer blocked and should be removed from
`EmitterConfig.BlockedClassWorkarounds`: `DataTextureLoader`, `InstancedInterleavedBuffer`, `Light`.

### Where a generated type is narrower than three.js

- **57 of the 224 generated classes have a narrower constructor.** 98 trailing optional parameters
...
| | classes |
|---|---|
| **generated, and typed** | **224** |
| reachable by name, untyped | 40 |
